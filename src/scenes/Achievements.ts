...
import {
  BADGES, TIER_ORDER, TIER_LABELS, fetchUnlockedAchievements,
  type BadgeDef, type BadgeTier, type BadgeIcon,
} from '../lib/achievements';

// ── Rarity palette — same six tiers/colors validated in the standalone
// design mockup, just ported from CSS custom properties into a plain TS
// lookup since this renders inside the game's DOM overlay, not a browser
// tab with its own <style> root. ──────────────────────────────────────────
...
// ── Icon paths — identical geometry to the validated design mockup. ───────
...
function badgeMedallionSVG(b: BadgeDef, uid: string, revealed: boolean): string {
  ...
  return `
    <svg width="120" height="120" viewBox="0 0 176 176" style="${locked && !secretHidden ? 'filter:saturate(0.35) brightness(0.7);' : ''}">
      <defs>
...
function badgeCardHTML(b: BadgeDef, idx: number, unlocked: boolean): string {
  const t = TIER_COLORS[b.tier];
  const revealed = unlocked; // secret badges only reveal name/art once unlocked
  const uid = b.id + idx;
  ...
  const unlockLine = b.secret && !unlocked
    ? '' // never reveal how to earn a secret badge before it's earned
    : `<div class="ach-unlock">${showComingSoon ? '🛠 Coming soon — not trackable yet' : b.unlockLabel}</div>`;
...
function injectAchievementStyles() {
  ...
  style.textContent = `
    .ach-body {
      ...
      background:
        radial-gradient(circle at 20% 8%, #241a33 0%, transparent 45%),
        radial-gradient(circle at 85% 75%, #1a2a3d 0%, transparent 45%),
        #100b18;
      padding: 20px 16px calc(28px + env(safe-area-inset-bottom,0px));
    }
    .ach-tier-head { display:flex; align-items:baseline; gap:8px; margin: 22px 0 16px; border-bottom:1px solid #2A2440; padding-bottom:8px; }
    .ach-tier-head:first-child { margin-top: 4px; }
    ...
    .ach-card { display:flex; flex-direction:column; align-items:center; text-align:center; padding:14px 8px; border-radius:16px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.04); }
    .ach-medallion-wrap { position:relative; width:120px; height:120px; display:flex; align-items:center; justify-content:center; margin-bottom:10px; }
    .ach-glow { position:absolute; inset:-14px; border-radius:50%; filter:blur(16px); z-index:0; }
    .ach-medallion-wrap svg { position:relative; z-index:1; }
    .ach-tag { display:inline-block; font-size:8.5px; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; padding:2.5px 8px; border-radius:20px; margin-bottom:6px; }
    .ach-tag-tier { font-size:8.5px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:6px; }
    .ach-name { font-family:'Fredoka',sans-serif; font-size:12.5px; font-weight:700; color:#F4EFFF; margin-bottom:4px; }
    .ach-stars { font-size:9px; letter-spacing:1.5px; color:#FFD23F; margin-bottom:6px; }
    .ach-quote { font-style:italic; font-size:10px; color:#B8AEDE; line-height:1.4; margin-bottom:6px; min-height:26px; }
    ...
    .ach-summary { text-align:center; margin-bottom:6px; color:#9E93C4; font-size:12px; }
  `;
...
export class Achievements extends Phaser.Scene {
  ...
  create() {
    ...
    shell.innerHTML = `<div class="dd-frame" id="achievements-frame" style="background:${c.bg};display:flex;flex-direction:column;"></div>`;
    this.containerEl = shell.querySelector('#achievements-frame') as HTMLDivElement;

    this.containerEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;flex-shrink:0;background:${c.bg};">
        <button id="btn-back" aria-label="Back" style="
        ...
        <span style="font-family:${theme.font.display};font-size:17px;font-weight:800;color:${c.textPrimary};">Achievements</span>
        ${soundToggleHTML('btn-sound-toggle', true)}
      </div>
      <div class="ach-body" id="ach-body">
        <div class="ach-summary" id="ach-summary">Loading your collection…</div>
      </div>
    `;
    ...
    this.containerEl.querySelector('#btn-back')?.addEventListener('click', () => {
      ...
      this.scene.start('Home', { audio: this.audio });
    });

    this.renderBadges();
  }

  private async renderBadges() {
    const identity = getIdentity();
    let unlocked = new Set<string>();
    if (identity && !identity.isGuest) {
      unlocked = await fetchUnlockedAchievements(identity.userId);
    }

    const body = this.containerEl?.querySelector('#ach-body') as HTMLElement;
    if (!body) return; // scene may have moved on before this resolved

    const total = BADGES.length;
    const earned = unlocked.size;
    const guestNote = !identity || identity.isGuest
      ? `<div class="ach-summary">Sign in to start earning badges — guests can browse but progress isn\u2019t saved.</div>`
      : `<div class="ach-summary">${earned} / ${total} badges earned</div>`;

    body.innerHTML = guestNote + TIER_ORDER.map(tierKey => {
